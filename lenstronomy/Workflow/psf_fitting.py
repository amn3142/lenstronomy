...
class PsfFitting(object):
    ...
    def update_psf(
        ...
        (
            ...
            cov_param,
            param,
        ) = self._image_model_class.image_linear_solve(**kwargs_params)
        ...
        (
            ...
            dec_image,
            point_amp,
        ) = self._image_model_class.PointSource.point_source_list(
    ...
    def image_single_point_source(self, image_model_class, kwargs_params):
        ...
        model, error_map, cov_param, param = image_model_class.image_linear_solve(
            **kwargs_params
        )
        ...
        kwargs_ps = kwargs_params.get("kwargs_ps", None)
        kwargs_lens = kwargs_params.get("kwargs_lens", None)
        point_source_list = self._point_sources_list(
            image_model_class, kwargs_ps, kwargs_lens
        )
    ...
    def _point_sources_list(image_model_class, kwargs_ps, kwargs_lens, k=None):
        ...
        (
            ...
            dec_array,
            amp_array,
        ) = image_model_class.PointSource.point_source_list(kwargs_ps, kwargs_lens, k=k)
        for i in range(len(ra_array)):
            point_source = image_model_class.ImageNumerics.point_source_rendering(
    ...
    def cutout_psf(
        ...
        for l in range(len(x)):
            mask_point_source = self.mask_point_source(
                ra_image, dec_image, ra_grid, dec_grid, radius, i=l
            )
            mask_i = mask * mask_point_source
            kernel_deshifted = self.cutout_psf_single(
                x[l], y[l], image_list[l], mask_i, kernel_size, kernel_init
            )
    ...
    def psf_estimate_individual(
        ...
        for l in range(len(ra_image)):
            ...
            residual_cutout = kernel_util.cutout_source(
                x_int, y_int, residuals, cutout_size + 2, shift=False
            )
            # cutout the mask
            mask_cutout = kernel_util.cutout_source(
                x_int, y_int, mask_i, cutout_size + 2, shift=False
            )
            # apply mask
            ...
            if supersampling_factor % 2 == 1:
                residuals_shifted = ndimage.shift(
                    residual_cutout_mask, shift=[shift_y, shift_x], order=1
                )

            else:
                # for even number super-sampling half a super-sampled pixel offset needs to be performed
                residuals_shifted = ndimage.shift(
                    residual_cutout_mask, shift=[shift_y - 0.5, shift_x - 0.5], order=1
                )
    ...
    def point_like_source_cutouts(x_pos, y_pos, image_list, cutout_size):
        ...
        for l in range(len(x_pos)):
            ...
            star_cutout = kernel_util.cutout_source(
                x_int, y_int, image_list[l], cutout_size, shift=False
            )
    ...
    def cutout_psf_single(x, y, image, mask, kernel_size, kernel_init):
        ...
        y_int = int(round(y))
        star_cutout = kernel_util.cutout_source(
            x_int, y_int, image, kernel_size + 2, shift=False
        )
        # cutout the mask
        mask_cutout = kernel_util.cutout_source(
            x_int, y_int, mask, kernel_size + 2, shift=False
        )
        ...
        shift_x = x_int - x
        shift_y = y_int - y
        kernel_shifted = ndimage.shift(
            kernel_enlarged, shift=[-shift_y, -shift_x], order=1
        )
        ...
        kernel_deshifted = kernel_util.de_shift_kernel(
            star_cutout, shift_x, shift_y, iterations=20, fractional_step_size=0.1
        )
        # re-size kernel
    ...
    def combine_psf(
        ...
        if keep_corners:
            ...
            angle_corner = 360.0 / corner_symmetry
        ...
        for kernel_new in kernel_list_new:
            ...
            if keep_corners:
                for j in range(corner_symmetry):
                    corner_kernel_rotated = image_util.rotateImage(
                        kernel_new, angle_corner * j
                    )
        ...
        kernel_new = kernel_util.kernel_norm(kernel_new)
        kernel_return = factor * kernel_new + (1.0 - factor) * kernel_old
    ...
    def error_map_estimate_new(
        ...
        for i, psf_kernel_i in enumerate(psf_kernel_list):
            kernel_low_i = kernel_util.degrade_kernel(
                psf_kernel_i, supersampling_factor
            )
    ...
    def error_map_estimate(
        ...
        for i, star in enumerate(star_cutout_list):
            ...
            C_D_cutout = kernel_util.cutout_source(
                x_int, y_int, self._image_model_class.Data.C_D, len(star), shift=False
            )
            # block neighbor points in error estimate
            mask_point_source = self.mask_point_source(
                x_pos, y_pos, ra_grid, dec_grid, radius=block_center_neighbour, i=i
            )
            mask_i = mask * mask_point_source
            mask_i = kernel_util.cutout_source(
                x_int, y_int, mask_i, len(star), shift=False
            )
    ...
    def mask_point_source(x_pos, y_pos, x_grid, y_grid, radius, i=0):
        ...
        for k in range(len(x_pos)):
            if k != i:
                mask_point = 1 - mask_util.mask_azimuthal(
                    x_grid, y_grid, x_pos[k], y_pos[k], radius
                )
                mask *= mask_point
